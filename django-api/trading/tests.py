...
class CandleAggregationTest(TestCase):
    ...
    # Сетка свечей выравнивается по абсолютной эпохе, а не по началу теста,
    # поэтому BASE обязан быть кратен самому большому таймфрейму (3600).
    # 1_700_000_000 таким не является (остаток 800) — бакеты разъедутся.
    BASE = 1_699_999_200
